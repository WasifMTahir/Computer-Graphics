...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  double y_new;
  if (num_samples == 1) {
    Ray r = camera->generate_ray(origin.x, origin.y); // replace 1.0 with your values based on x,y,sampleBuffer.w,sampleBuffer.h
    r.depth = max_ray_depth;
    output = trace_ray(r, true);
    sampleCountBuffer[index] = num_samples;
  }
  for (int i=0; i<ns_aa; i++) {
    Vector2D currSample = gridSampler->get_sample();
    x_new = origin.x - (currSample.x/2);
    y_new = origin.y - (currSample.y/2);
    Ray r = camera->generate_ray(x_new, y_new);
    output = output + trace_ray(r, true);
  }
  return output/ns_aa;
