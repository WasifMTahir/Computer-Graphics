...
Spectrum PathTracer::estimate_direct_lighting(const Ray& r, const Intersection& isect) {
  ...
  for (SceneLight* currLight : scene->lights) {
      int numSamples = 1;
      if (!currLight->is_delta_light())
          numSamples = ns_area_light;
      for (int i=0; i<numSamples; i++) {
          const Spectrum& light = currLight->sample_L(hit_p, &wi, &distToLight, &pdf);
          Vector3D w_in = w2o * wi;
          if (w_in.z < 0)
              continue;
          if (!bvh->intersect(Ray(EPS_D * wi + hit_p, wi, distToLight))) {
              const Spectrum& bsdfVal = isect.bsdf->f(w_out, w_in);
              L_out = L_out + (w_in.z/(numSamples*pdf)) * bsdfVal * light;
...
Spectrum PathTracer::estimate_indirect_lighting(const Ray& r, const Intersection& isect) {
  ...
  Vector3D w_in;
  float pdf;
  const Spectrum& bsdfVal = isect.bsdf->sample_f(w_out, &w_in, &pdf);
  float rrProb = 1 - bsdfVal.illum();
  if (coin_flip(rrProb))
      return L_out;
  Ray indLight(EPS_D * o2w * w_in + hit_p, o2w * w_in, INF_D, r.depth - 1);
  return L_out + (fabs(w_in.z)/(pdf * (1 - rrProb))) * bsdfVal * trace_ray(indLight, isect.bsdf->is_delta());
}
